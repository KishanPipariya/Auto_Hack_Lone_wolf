...
def get_user_history(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    history = db.query(ItineraryHistory).filter(ItineraryHistory.user_id == current_user.id).order_by(ItineraryHistory.created_at.desc()).all()
...
def get_history_detail(history_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    item = db.query(ItineraryHistory).filter(ItineraryHistory.id == history_id, ItineraryHistory.user_id == current_user.id).first()
    if not item:
    ...
    logger.info(f"Returning history item {history_id} with keys: {list(blob.keys()) if isinstance(blob, dict) else 'NotDict'}")
...
def save_history(item: HistoryCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    import logging
    logger = logging.getLogger("travel_agent_server")
    logger.info(f"POST /history received from user {current_user.email} for city {item.city}")
