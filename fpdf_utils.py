...
async def generate_pdf(itinerary: Itinerary):
    ...
    destination = itinerary.recommended_destination or itinerary.city
    pdf.cell(0, 10, f"Trip to {destination}", new_x="LMARGIN", new_y="NEXT", align="C")
    pdf.ln(5)
    ...
    total_str = f" Total Estimated Cost: ${itinerary.cost_breakdown.total or itinerary.total_cost} "
    ...
    pdf.cell(0, 6, "Trip Summary", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("helvetica", "", 10)
    pdf.set_text_color(55, 65, 81)
    if itinerary.vibe_rationale:
        pdf.multi_cell(0, 5, f"Why this destination: {itinerary.vibe_rationale}")
    if itinerary.work_friendly_notes:
        pdf.multi_cell(0, 5, f"Work-friendly notes: {itinerary.work_friendly_notes}")
    if itinerary.budget_notes:
        pdf.multi_cell(0, 5, f"Budget notes: {itinerary.budget_notes}")
    ...
    for label, value in [
        ...
        pdf.cell(38, 7, f"{label}: ${value}", border=1, fill=True, align="C")
    pdf.ln(12)
    ...
    return Response(
        ...
        headers={"Content-Disposition": f"attachment; filename=Trip_to_{destination}.pdf"}
    )
