...
def api_request(method, endpoint, data=None, token=None):
    ...
    try:
        ...
        st.error(
            f"❌ Could not connect to backend at {API_URL}. Is the server running?"
        )
        return None
        ...
        if submitted:
            ...
            if custom_interests:
                final_interests.extend(
                    [x.strip() for x in custom_interests.split(",") if x.strip()]
                )
...
def render_history():
    ...
    if res and res.status_code == 200:
        ...
        for trip in trips:
            with st.expander(
                f"🌍 {trip['city']} ({trip['days']} Days) - {trip['created_at'][:10]}"
            ):
                # Need to fetch details
                if st.button("Load Details", key=f"btn_{trip['id']}"):
                    detail_res = api_request(
                        "GET", f"/history/{trip['id']}", token=st.session_state.token
                    )
